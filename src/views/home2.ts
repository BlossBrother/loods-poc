...
export interface LoodsHomeData {
  ...
  badges: { prikbord: number; meldingen: number };
  pushKey: string;
}
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  .hdr{position:sticky;top:0;z-index:40;padding:calc(env(safe-area-inset-top) + 14px) 18px 12px;backdrop-filter:saturate(160%) blur(18px);-webkit-backdrop-filter:saturate(160%) blur(18px);background:var(--hdr-bg);transition:background-color .25s ease}
  ...
  .chip{display:flex;align-items:center;gap:7px;flex:none;padding:7px 13px 7px 10px;border-radius:999px;background:var(--card);border:1px solid var(--line);box-shadow:var(--sh-soft);font-size:12.5px;font-weight:600;color:var(--ink);cursor:pointer}
  ...
  main{padding:6px 18px calc(env(safe-area-inset-bottom) + 128px)}
  ...
  .wabar{position:fixed;left:50%;transform:translateX(-50%);bottom:calc(env(safe-area-inset-bottom) + 12px);z-index:50;width:calc(100% - 24px);max-width:440px;background:var(--cap-bg);backdrop-filter:saturate(150%) blur(22px);-webkit-backdrop-filter:saturate(150%) blur(22px);border:1px solid var(--cap-bd);border-radius:40px;padding:7px;display:flex;box-shadow:var(--cap-shadow)}
  .blob{position:absolute;top:7px;left:0;height:calc(100% - 14px);border-radius:28px;background:var(--cap-blob);transform:translateX(var(--bx,0));width:var(--bw,60px);transition:transform .4s cubic-bezier(.22,.72,.24,1),width .4s cubic-bezier(.22,.72,.24,1);pointer-events:none}
  ...
  .up{opacity:0;transform:translateY(14px);animation:rise .55s cubic-bezier(.2,.7,.2,1) forwards;animation-delay:var(--d,0s)}
  @keyframes rise{to{opacity:1;transform:none}}
  @media (prefers-reduced-motion: reduce){
    .up{animation:none;opacity:1;transform:none}
    .rail::after,.bhv .tile{animation:none}
    .blob,.press,.wtab{transition:none}
  }
...
(function(){
  ...
  if(bar){
    ...
    // Blob glijdt mee bij een tik (zelfde gevoel als de shell-tabbar). Home is een
    // volledige paginalaad; navigatie heel even uitstellen zodat de glide afmaakt.
    bar.addEventListener("click",function(e){
      var t=e.target.closest?e.target.closest("a.wtab"):null;
      if(!t)return;
      place(t);
      if(t.href&&!t.classList.contains("on")){
        e.preventDefault();
        var href=t.href;
...
export function loodsHome(d: LoodsHomeData) {
  let delay = 0.05;
  const dly = () => { const v = delay.toFixed(2); delay += 0.05; return v; };
...
${raw('<script>(function(){try{var t=localStorage.getItem("ff_theme");if(t==="dark"||t==="light")document.documentElement.setAttribute("data-theme",t);}catch(e){}})();</script>')}
<style>${raw(CSS)}</style>
</head>
<body>
${raw(SPRITE)}
...
<div class="app">
  <header class="hdr" id="hdr">
    <div class="hdr-row">
      ...
      <a class="iconbtn ava press" href="/mijn-account" aria-label="Mijn profiel">${d.profielFoto ? html`<img src="${d.profielFoto}" alt="" width="44" height="44" loading="lazy" />` : d.initialen}</a>
    </div>
    <div class="chips">
  ...
  <main>
    ${d.alarm
      ...
      : ""}
    ${!d.alarm && d.vandaag.length === 0 && !d.kantine && d.voorJou.length === 0
  ...
  <nav class="wabar" id="wabar" aria-label="Hoofdnavigatie">
    <span class="blob" id="blob"></span>
    <a class="wtab on" href="/" aria-current="page">${ico("home")}<span>Home</span></a>
    <a class="wtab" href="/agenda">${ico("cal")}<span>Agenda</span></a>
    <a class="wtab" href="/social">${ico("msg")}<span>Prikbord</span>${d.badges.prikbord > 0 ? html`<span class="tbadge">${d.badges.prikbord}</span>` : ""}</a>
    <a class="wtab" href="/meldingen">${ico("bell")}<span>Meldingen</span>${d.badges.meldingen > 0 ? html`<span class="tbadge">${d.badges.meldingen}</span>` : ""}</a>
